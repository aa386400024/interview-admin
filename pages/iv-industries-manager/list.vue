<template>
	<view class="page-body">
		...
		<vk-data-table
			...
			:default-sort="{ name:'industry_id', type:'asc' }"
			:selection="true" 
			:row-no="true" 
			:pagination="true"
			@update="updateBtn" 
	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/iv-industries-manager/sys/getList",
					// 表格字段显示规则
					columns: [{
							key: "_id",
							title: "id",
						...
						},
						{
						    ...
						    formatter: (val, row, column, index) => {
						        if (val.isDot) {
						            return '<span style="color: red; font-weight: bold;">● 红点</span>';
						        } else if (val.value > 0) {
						            return `<span style="color: blue; font-weight: bold;">${val.value} 数字</span>`;
						        } else {
						            return '<span>无</span>';
						        }
						    }
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/iv-industries-manager/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				that.form1.props.action = 'admin/iv-industries-manager/sys/update';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/iv-industries-manager/sys/delete",
					data: {
						_id: item._id
					},
				});
