<template>
	<view class="page-body">
		...
		<!-- 职位技能弹窗 -->
		<vk-data-dialog 
		    v-model="isPositionsStackDialogVisible" 
		    title="职位技能列表" 
		    width="60%">
		    <vk-data-table
		        :data="selectedPositionStack"
		        :columns="positionStackTable.columns">
		    </vk-data-table>
		</vk-data-dialog>
		...
		<vk-data-table
			...
			:default-sort="{ name:'_id', type:'asc' }"
			:selection="true" 
			:row-no="true" 
			:pagination="true"
			@update="updateBtn" 
...
<script>
	...
	export default {
		data() {
			// 页面数据变量
			return {
				selectedPositionStack: [], // 用于存储当前选中的职位技能列表
				isPositionsStackDialogVisible: false, // 控制职位对话框的显示/隐藏
				// 页面是否请求中或加载中
				loading: false,
				// init请求返回的数据
				data: {},
				...
				table1: {
					// 表格数据请求地址
					action: "admin/iv-tech-stack-manager/sys/getList",
					// 表格字段显示规则
					columns: [{
							key: "_id",
							title: "id",
				...
				form1: {
					...
					props: {
						...
						columns: [{
							...
							"title": "职位技能名称",
							"type": "text",
							"placeholder": "请输入职位技能名称",
							"showLabel": true
						}, {
							"key": "tech_name_zh",
							"title": "职位技能英文名称",
							"type": "text",
							"placeholder": "请输入职位技能英文名称",
							"showLabel": true
						}, {
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/iv-tech-stack-manager/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
			    that.form1.props.action = 'admin/iv-tech-stack-manager/sys/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/iv-tech-stack-manager/sys/delete",
					data: {
						_id: item._id
					},
				});
			...
			showPositions(index) {
				this.selectedPositionStack = this.table1.columns[index].positions;
				this.isPositionsStackDialogVisible = true;
			}
		},
		// 监听属性
		watch: {},
